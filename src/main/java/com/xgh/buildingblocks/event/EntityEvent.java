...
public abstract class EntityEvent<IdT extends EntityId> implements Event {
    @JsonIgnore
    ...
    protected EntityEvent(IdT entityId, EntityVersion entityVersion) {
     ...
     */
    public static EntityEvent<?> fromJson(
            String eventType,
            UUID entityId,
            ...
            String data) {
        try {
            EntityEvent<?> event = (EntityEvent<?>) unserialize(eventType, data);
     ...
     */
    private static Field getEntityIdField(EntityEvent<?> event) throws NoSuchFieldException {
        Field field;
